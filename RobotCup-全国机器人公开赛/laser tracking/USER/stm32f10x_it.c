...
#define RDATA PEin(4)	 //红外数据输入脚
void Sys_Soft_Reset(void);      //系统软复位
extern u32 TIM2_num;
extern unsigned long	lEncoderTotal;          /* pulse累计值                    */
extern unsigned int    iEncoderMax;            /* pulse最大值                */
extern unsigned int    iEncoder;               /* 现在值                    */
...
* Return         : None
*******************************************************************************/
void EXTI1_IRQHandler(void)			 //摄像头
{
...
* Return         : None
*******************************************************************************/
void EXTI2_IRQHandler(void)	  //外部中断1入口函数
{
}
...
* Return         : None
*******************************************************************************/
void EXTI3_IRQHandler(void)	 //外部中断2入口函数
{
...
* Return         : None
*******************************************************************************/
void TIM2_IRQHandler(void)	  //10ms中断一次
{
	unsigned int i;
	...
	/* 编码器关联 */  //编码器脉冲采样周期10ms
	i = TIM_GetCounter(TIM1);  //i = Code_CNT;
	if(i>100) i = 0;
	TIM_SetCounter(TIM1, 0);   // 清零计数器CNT  Code_CNT = 0;
	iEncoder       = i;
	lEncoderTotal += iEncoder;
	if( iEncoder > iEncoderMax )  iEncoderMax = iEncoder;
}  
